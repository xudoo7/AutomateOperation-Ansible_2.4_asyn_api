    ...
    def gen_inventory(self):
        ...
        if 'hosts' in self.resource.keys():
            for host, info in self.resource['hosts'].items():
                ...
                #print(obj_host)
                self.inventory.add_host(obj_host)
                #self.inventory.get_groups_dict()
                #print(self.inventory.list_hosts())
                #for host in self.inventory.get_hosts('all'):
                #    print(host)
                #print("ok")
        # add group
            ...
            for group, detail in self.resource['groups'].items():
                ...
                if 'child' in detail.get('child', []):
                    for child in detail.get('child', []):
                        ...
                        self.inventory.add_child_group(self.inventory.get_group(child))

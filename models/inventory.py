...
class BaseHost(Host):
    def __init__(self, host_data):
        ...
        #print(type(host_data))
        #print(host_data.get('hostname'))
        #hostname = host_data.get('ip') or host_data.get('hostname')
        #hostname = host_data['hostname'] or host_data['ip']
        #print(hostname)
    ...
    def __set_required_variables(self):
        ...
        self.set_variable('ansible_host', host_data.get('ip'))
        self.set_variable('ansible_port', host_data.get('port'))

        if host_data.get('username'):
            self.set_variable('ansible_user', host_data.get('username'))

        # 添加密码和秘钥
        if host_data.get('password'):
            self.set_variable('ansible_ssh_pass', host_data.get('password'))
        if host_data.get('private_key'):
            self.set_variable('ansible_ssh_private_key_file', host_data.get('private_key'))
...
class BaseInventory(InventoryManager):
    ...
    def __init__(self, host_list=None):
       ...
       #print host_list
        ...
        #print(isinstance(host_list, list))
#        assert isinstance(host_list, list)
        self.loader = self.loader_class()
        self.variable_manager = self.variable_manager_class()
        super().__init__(self.loader) 
        #super(BaseHost, self).__init__(self.loader) 
    ...
    def parse_sources(self, cache=False):
        ...
        for host_data in self.host_list:
            ...
            #print(host)
            self.hosts[host_data.get('hostname')] = host
